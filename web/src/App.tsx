...
async function login(username: string, password: string): Promise<{ token: string; user: User }> {
...
const SeverityBadge: React.FC<{ severity: string }> = ({ severity }) => {
  ...
  return (
    <span className={`px-2 py-1 rounded text-white text-xs font-medium ${colors[severity] || 'bg-gray-500'}`}>
...
const StatusBadge: React.FC<{ status: string }> = ({ status }) => {
  ...
  return (
    <span className={`px-2 py-1 rounded text-white text-xs font-medium ${colors[status] || 'bg-gray-500'}`}>
      {status}
    </span>
  );
};
  ...
  <div className="bg-gray-800 rounded-lg p-4 shadow-lg">
    ...
    <div className="space-y-3">
      <div className="flex justify-between items-center">
        ...
        <span className="text-2xl font-bold text-red-400">{stats.critical_alerts}</span>
      </div>
      <div className="flex justify-between items-center">
        <span className="text-orange-400">High</span>
        <span className="text-2xl font-bold text-orange-400">{stats.high_alerts}</span>
      </div>
      <div className="flex justify-between items-center">
        <span className="text-yellow-400">Medium</span>
        <span className="text-2xl font-bold text-yellow-400">{stats.medium_alerts}</span>
      </div>
      <div className="flex justify-between items-center">
        <span className="text-blue-400">Low</span>
        <span className="text-2xl font-bold text-blue-400">{stats.low_alerts}</span>
      </div>
    </div>
  </div>
);

const ValidatorHealth: React.FC<{ online: number; offline: number }> = ({ online, offline }) => {
  const total = online + offline;
  const healthPercent = total > 0 ? (online / total) * 100 : 0;
  return (
    <div className="bg-gray-800 rounded-lg p-4 shadow-lg">
      <h3 className="text-white text-lg font-semibold mb-4">Validator Health</h3>
      <div className="relative pt-1">
        ...
        <p className="text-gray-400 text-sm">{healthPercent.toFixed(1)}% Healthy</p>
      </div>
    </div>
  );
};
      ...
      <div className="flex items-center justify-center">
        <div className="relative w-32 h-32">
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <span className={`text-xl font-bold ${color} uppercase`}>{level}</span>
          </div>
        </div>
      </div>
    </div>
...
const ComplianceScore: React.FC<{ score: number }> = ({ score }) => {
  ...
  return (
    <div className="bg-gray-800 rounded-lg p-4 shadow-lg">
      <h3 className="text-white text-lg font-semibold mb-4">Compliance Score</h3>
      <div className="flex items-center justify-center">
        <div className={`text-5xl font-bold ${getColor(score)}`}>{score.toFixed(1)}%</div>
      </div>
      <p className="text-gray-400 text-sm text-center mt-2">SOC 2 / ISO 27001</p>
    </div>
  );
};
...
const RecentIncidents: React.FC<{ incidents: IncidentBrief[] }> = ({ incidents }) => (
  <div className="bg-gray-800 rounded-lg p-4 shadow-lg">
      ...
      {incidents.map((incident) => (
        <div key={incident.id} className="border-l-4 border-gray-600 pl-3 py-1">
          <div className="flex justify-between items-start">
            <span className="text-gray-300 text-sm font-medium">{incident.title}</span>
...
const Sidebar: React.FC<{ activeLayout: string; layouts: Layout[]; onLayoutChange: (id: string) => void }> = ({
  activeLayout,
  layouts,
  onLayoutChange,
}) => (
  <aside className="w-64 bg-gray-900 border-r border-gray-800 p-4">
    <nav className="space-y-2">
      <h2 className="text-gray-400 text-xs uppercase tracking-wider mb-4">Dashboards</h2>
      {layouts.map((layout) => (
        <button
          key={layout.id}
    ...
    <div className="mt-8 space-y-2">
      ...
      {['Alerts', 'Events', 'Validators', 'Compliance', 'Reports', 'Settings'].map((item) => (
        <button
          key={item}
  ...
  return (
    <div className="min-h-screen bg-gray-900 flex items-center justify-center">
      <div className="bg-gray-800 p-8 rounded-lg shadow-xl w-96">
        <h1 className="text-2xl font-bold text-white text-center mb-6">Boundary SIEM</h1>
        ...
        <p className="text-gray-500 text-xs text-center mt-4">Supports OAuth, SAML, and LDAP</p>
      </div>
    </div>
  );
};
...
const Dashboard: React.FC = () => {
  ...
  const loadData = useCallback(async () => {
    try {
      const [statsData, layoutsData] = await Promise.all([fetchStats(), fetchLayouts()]);
      setStats(statsData);
  ...
  return (
    <div className="min-h-screen bg-gray-900 flex flex-col">
      ...
      <div className="flex flex-1">
        <Sidebar activeLayout={activeLayout} layouts={layouts} onLayoutChange={setActiveLayout} />
            ...
            <div className="grid grid-cols-12 gap-4">
              ...
              <div className="col-span-3">
                <MetricCard
                  ...
                  subtitle="Healthy"
                  color={stats.nodes_unhealthy > 0 ? 'text-yellow-400' : 'text-green-400'}
                />
              </div>
              <div className="col-span-3">
                <MetricCard title="Compliance" value={`${stats.compliance_score}%`} color="text-green-400" />
              </div>
              ...
              <div className="col-span-3">
                <ValidatorHealth online={stats.validators_online} offline={stats.validators_offline} />
              </div>
              <div className="col-span-3">
